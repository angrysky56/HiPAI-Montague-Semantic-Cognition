...
@pytest.fixture
def manager():
    m = HIPAIManager(graph_name="test_quantifiers")
    m.world_model.clear_graph()
    return m
...
def test_negative_universal(manager):
    ...
    # Verify Concept_Dog has prop_not_cats = true
    res = manager.world_model.query_graph("MATCH (c:Concept {name: 'Concept_Dog'}) RETURN c.prop_not_cats")
...
def test_singular_quantifiers(manager):
    ...
    res = manager.world_model.query_graph("MATCH (c:Concept {name: 'Concept_Dog'}) RETURN c.prop_not_bird")
    assert res[0][0] is True
