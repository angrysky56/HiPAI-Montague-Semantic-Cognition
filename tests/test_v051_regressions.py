...
def test_nationality_lemmatization(wm):
    ...
    res = wm.query_graph("MATCH (n:Entity {id: 'diogenes'})-[:INSTANCE_OF]->(c) RETURN c.name")
    assert res == [['Concept_Greek']]
    
    # Verify only ONE Greek concept exists
    res = wm.query_graph("MATCH (c:Concept) WHERE c.name CONTAINS 'Greek' RETURN c.name")
    assert len(res) == 1
    assert res[0][0] == "Concept_Greek"
    
    # Verify Diogenes -> Greek -> Philosopher chain (transitive subsumption)
    res = wm.query_graph("MATCH (n:Entity {id: 'diogenes'})-[:INSTANCE_OF]->(:Concept {name: 'Concept_Greek'})-[:SUBCLASS_OF]->(p:Concept) RETURN p.name")
