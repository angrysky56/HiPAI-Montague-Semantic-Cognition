...
def test_attitude():
    ...
    for rel in obs.relations:
        print(f"  Relation: {rel.source_id} -[{rel.relation_type}]-> {rel.target_id or 'NESTED'}")
        if rel.target_observation:
            print(f"    Nested Obs Individuals: {[ind.name for ind in rel.target_observation.individuals]}")
            ...
            for n_rel in rel.target_observation.relations:
                print(f"      Nested Relation: {n_rel.source_id} -[{n_rel.relation_type}]-> {n_rel.target_id}")
