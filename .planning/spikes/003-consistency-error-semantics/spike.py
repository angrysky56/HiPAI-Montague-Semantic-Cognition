...
def test_world_cloning_recovery():
    ...
    with onto:
        ...
        class Restricted(Action): pass
        class Permitted(Action): pass
...
def test_inference_under_relaxation():
    ...
    with onto:
        class Action(Thing): pass
        class HARM(Action): pass
        class DECEIVE(Action): pass
        class Permitted(Action): pass
    ...
    try:
        ...
        found_culprits = []
        if isinstance(act1, RestrictedHARM): found_culprits.append("HARM_VIOLATION")
        if isinstance(act2, RestrictedDECEIVE): found_culprits.append("DECEIVE_VIOLATION")
